...
const categories = require('../src/categoriesStore')
const questions = require('../src/questionStore')
...
const boot = () => {
  ...
  InterfaceInstance.question('\n\n**TO HAVE THE MOST FUN, MAKE YOUR TERMINAL FULL SCREEN**\nType 1, 2, or 3 and hit enter:\n\n1 to play Terminal Trivia\n2 to learn how to play Terminal Trivia\n3 to Meet the Developer\n\nType 1, 2, or 3 \n$ ', (option) => {
    //Play Jeopardy
    if(option === '1'){
      categories.categoriesStore
        ...
        */
        return questions.questionStore(categoriesArr)
          .then((gameDataObj) => {
            /*
                gameDataObject = {
                'that\'s just ducky':
    ...
    //The Rules of Terminal-Triva
      ...
      console.log("  \n3. Case (upper or lower) is not important but please try to spell correctly");
      console.log("  \n4. Enjoy the game!");
      setTimeout(() => {
        boot();
      },5000);
      ...
      console.log("\n\n\nMeet The Developer");
      console.log("-------------------");
      console.log('https://www.danieldwyer.net');
      setTimeout(() => {
        boot();
      },3000);
    }else{
...
const gameplay = (gdo) => {
  gameBoard.gameBoard(gdo, InterfaceInstance)
    .then((dynamicGameData) => {
      // console.log("dynamicGameData:",util.inspect(dynamicGameData, {showHidden: false, depth: null, color: true}))
      // const wishes = ['You Got This!', 'Show Me What You Got!', 'Never Give Up!', 'Next Stop, The Top!',]
      // console.log("\nYou Got This!\n");
      /*
          dynamicGameData = {f
      ...
      */
      ...
      new Promise((resolve, reject) => {
        ...
        InterfaceInstance.question('\nWhich Category? To choose, type the number next to the category and hit enter (DO NOT TYPE CATEGORY NAME)\n$ ', (category) => {
          if((Number(category).toString()) == 'NaN' || Number(category) > 6){
            ...
            InterfaceInstance.question('\nFor What Value? To choose, just type the value and hit enter\n$ ', (value) => {
              if(value == 200){
                const clueArrIndex = 0
                resolve({category: category, clueArrIndex: clueArrIndex})
              }else if(value == 400){
                const clueArrIndex = 1
                resolve({category: category, clueArrIndex: clueArrIndex})
              }else if(value == 600){
                const clueArrIndex = 2
                resolve({category: category, clueArrIndex: clueArrIndex})
              }else if(value == 800){
                const clueArrIndex = 3
                resolve({category: category, clueArrIndex: clueArrIndex})
              }else if(value == 1000){
                const clueArrIndex = 4
                resolve({category: category, clueArrIndex: clueArrIndex})
              }else{
        ...
        .then((userInput) => {
          ...
          for(let keys in dynamicGameData){
            if(dynamicGameData[keys].localID == userInput.category){
              if(dynamicGameData[keys]['clues'][userInput.clueArrIndex].used === undefined){
                ...
                const question = dynamicGameData[keys]['clues'][userInput.clueArrIndex].question;
                const answerCorrect = dynamicGameData[keys]['clues'][userInput.clueArrIndex].answer.toLowerCase().replace('<i>', '').replace('</i>', '');
                console.log("\nQ:",question);
